...
A run works through three layers, stopping at the first that fails:
...
`per-file` is simpler to write and costs N files × M checks invocations.
`per-cluster` walks the tree itself and is much cheaper on a large lab.

| `format`    | Contract                                                                                                                                      |
| ----------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| `exit-code` | non-zero fails. Stdout is the message                                                                                                         |
| `json`      | stdout is `[{severity, resource, message}]`. Empty array passes. **Exit code ignored**. Per-diagnostic severity overrides the check's default |

Each check becomes a `writeShellApplication` with `yq`, `jq` and coreutils
on PATH, symlinked into the lab package at `$out/lint/<name>`.

> A check that fails to _execute_, non-executable, missing interpreter,
> missing dependency, is reported as an error naming the check. It used to
> be swallowed, which made a broken check indistinguishable from a passing
> one.
...
- [How It Works](../understanding/how-it-works.md): where lint sits among
  the other layers.
- [Images and Registries](./images.md): what `image-pin` enforces.
