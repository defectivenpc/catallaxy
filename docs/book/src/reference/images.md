...
Three related concerns: pinning images so a deploy is reproducible, getting
them onto a machine that may not have internet, and publishing images the
lab builds itself.
...
Both are enforced by the `image-pin` [lint rule](./lint.md), not at apply
time, you find out in CI, not during a rollout.
...
`lab.images.pins.<name>.ref` is the assembled reference, use that rather
...
- [Lint Rules](./lint.md): where the policy is enforced.
- [Module Options](./options.md): the lab option tree.
