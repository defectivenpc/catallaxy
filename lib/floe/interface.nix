...
#
# This is the typeclass, and it is deliberately *portable* — nothing in this
# file knows what Kubernetes is. What a cluster floe additionally has
# (`bundles`, `namespace`, `images`, `network`, …) lives in `./cluster.nix`,
# and what a lab floe additionally has lives in `./lab.nix`. Both are handed
# to `extensions` and merged into every instance.
#
# That split is not a tidiness exercise; it is the finding that made the lab
# scope tractable. `modules/lab/lab-floe-options.nix` and the cluster's
# `floe-options.nix` declared seven of the same channels twice — `enable`,
# `version`, `infra.resources`, `steps`, `ops`, `lint`, `verify` — with the
# lab's set a strict subset of the cluster's by channel name. Two hand-copied
# interfaces at two dynamic paths is the same antipattern the whole exercise
# exists to remove, just duplicated at a second scope. They are one type with
# two extensions.
#
# The seam is upstream's. `lib/services/lib.nix`'s `configure` takes
# `extraRootModules` for exactly this: a portable base plus whatever the
# containing system adds, "loaded into the root service submodule, but not
# into its sub-services."
#
# ## Composition
#
# `options.floes` here is the composition mechanism, and it is the reason a
# lab floe is not a different kind of thing: a lab floe is a floe that has
# sub-floes. `lib/services/service.nix:25` declares `options.services` inside
# `service.nix` in precisely this shape, and the manual says why:
#
#   "Services can be compositions of other services. […] it is a good practice
#    when developing services to first write them as individual services, and
#    then compose them into a higher-level composition. Each of these services
#    is a valid modular service, including their composition."
#
# Recursion also buys the thing this spike is for: core functionality is
# written once as a tree walk (`./fold.nix`) and works at every depth, so an
# author who writes `assertions = [ … ]` in an instance gets lab-level
# reporting with no per-floe wiring anywhere.
#
# ## Why framework values arrive through `importApply`
#
# `args` — `pkgs`, `cataCharts`, `k8sSpecs`, `lab`, `cluster`, `contracts` —
# is baked into this module at import time rather than delivered as
# `_module.args` by whoever declares the option. It has to be, and the reason
# is the recursion: setting `config.floes.<k>._module.args` from inside this
# module would define `config.floes`'s attribute *names* in terms of
# `config.floes`'s attribute names, which does not converge. Upstream has the
# same constraint and the same answer — `importApply ./service.nix { inherit
# pkgs; }`, self-referenced by path.
#
# So the values go into the submodule *type* instead, where they close over
# nothing that depends on them. `peers` is the one exception and it is
# handled the same way: it closes over `config.floes` lazily, which is fine
# because `mapAttrs` does not force its values and no floe reads its own
# exports through its own peers.
{
  modulesPath,
  args ? { },
  extensions ? [ ],
