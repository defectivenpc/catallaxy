# gateway's own options, as an instance of the floe interface.
#
# The port of ./options.nix. Every difference is one of three mechanical ones:
#
#   - `options.floes.gateway = { ... }` becomes `options = { ... }`, because
#     `config` is this floe;
#   - `config.floes.gateway.tls.domain` becomes `config.tls.domain`;
#   - `config.floes.cert-manager.exports.issuance` becomes
#     `peers.cert-manager.issuance` — the one cross-floe read in this file, and
#     the reason `peers` had to work at option-*default* time rather than only
#     inside `config`.
{
  lab,
  config,
...
in
{
  ...
  config.namespace = mkDefault "kube-system";
  ...
  options = {
    ...
    tls = {
      enable = mkOption {
        type = types.bool;
        default = config.tls.domain != "" && (peers.cert-manager.issuance or null) != null;
      ...
      extraCertificateRefs = mkOption {
        ...
        description = ''
          ...
          Contributed by other floes today, which under the interface is
          the one thing a sibling cannot do: `peers` carries `exports`
          and carries them one way. See `internalHostnames` below — same
          gap, same open question.
        '';
      };
    ...
    # The one thing this port does not answer.
    #
    # Four floes — kanidm, forgejo, netbird, prometheus — write
    # `floes.gateway.internalHostnames` today. That is a sibling writing
    # another floe's option, and it works only because every floe shares one
    # flat `floes.*` namespace. Under the interface it is exactly what `peers`
    # refuses: exports flow outward, and there is no inbound channel.
    #
    # Left as gateway's own option here so the A/B comparison is about the
    # port and not about a channel that does not exist yet. The shape it
    # probably wants is the one `cluster.registryDomains` already has — floes
    # contribute to a cluster-level list, gateway reads it off the `cluster`
    # argument — because "hostnames on the internal tier" is a fact about the
    # cluster, not a setting on the gateway. Decide it when the registry is
    # wired into the cluster, not before.
