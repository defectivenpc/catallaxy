# One lab, stood up from nothing and torn back down to nothing.
#
# The assertion that matters is not that a lab comes up — it is that running
# `lab up` a second time against an unchanged declaration does nothing at all,
# and that teardown leaves nothing behind. Both are invisible to any check over
# rendered output, and both have been wrong here before.
{
  lib,
  pkgs,
  cataWrapped,
...
pkgs.writeShellApplication {
  ...
  runtimeInputs = [
    ...
    # The parked script shelled out to `nix eval` without declaring it, which
    # it got away with because CI always had nix on PATH and a workstation
    # usually does too. It is a dependency either way.
    pkgs.nix
  ];
  ...
  text = ''
    ...
    # Re-checked here rather than trusted from the caller, so a name that does
    # not belong in a CI matrix fails loudly on its own job instead of being
    # silently dropped from a computed list.
    ...
    # Everything this lab owns, and nothing another one does. Host services are
    # `catallaxy-<lab>-<svc>`; k3d nodes are `k3d-<prefix>-...`, where the
    # prefix is the lab name with separators flattened.
    #
    ...
    # A stack's state is the only record of what it made. Teardown claiming the
    # lab is gone while state still lists resources would be a claim about a
    # cloud account nobody checked. Nothing declares infra yet; the check costs
    # nothing and is here so it is not forgotten when something does.
